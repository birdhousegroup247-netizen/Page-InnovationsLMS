 ...
 * Sidebar
 *
 * Renders a hybrid navigation: flat items at the top level (one-click access
 * to daily-used pages) and collapsible groups for less-frequent sections.
 *
 ...
 *   - The group whose child matches the current route is auto-expanded on
 *     load even if localStorage has it collapsed.
 */

const LS_KEY = 'admin.sidebar.openGroups.v1';
...
function filterNavigation(items) {
  ...
  for (const entry of items) {
    if (entry.type === 'item') {
      if (isFeatureOn(entry.feature)) out.push(entry);
...
const Sidebar = ({ items = [], className, isOpen = false, onClose }) => {
  ...
  // Per-group expand state, persisted. The group containing the active
  // route gets auto-expanded on first match so the active item is reachable.
