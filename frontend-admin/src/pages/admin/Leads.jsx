...
const STATUS_LABELS = {
  ...
  welcome_sent: { label: 'Welcome Sent', color: 'bg-blue-900/40 text-blue-400' },
  d1_sent: { label: 'D1 Sent', color: 'bg-blue-900/40 text-blue-400' },
  d3_sent: { label: 'D3 Sent', color: 'bg-indigo-900/40 text-indigo-400' },
  d7_sent: { label: 'D7 Sent', color: 'bg-purple-900/40 text-purple-400' },
  d14_sent: { label: 'D14 Sent', color: 'bg-purple-900/40 text-purple-400' },
  converted: { label: 'Converted', color: 'bg-green-900/40 text-green-400' },
  unsubscribed: { label: 'Unsubscribed', color: 'bg-red-900/40 text-red-400' },
};
    ...
    <>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-dark-800 border border-dark-700 rounded-xl p-4">
            <p className="text-xs text-text-secondary mb-1">Converted</p>
          ...
          <div className="bg-dark-800 border border-dark-700 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-purple-400">{stats.recentLeads}</p>
          </div>
        </div>
      )}
      ...
      {stats?.byStatus && (
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-4 mb-6">
          <p className="text-sm font-medium text-text-secondary mb-3 flex items-center gap-1.5"><TrendingUp className="w-4 h-4" /> Drip Funnel</p>
            ...
            {stats.byStatus.filter(s => s.status !== 'unsubscribed').map((s) => {
              ...
              return (
                <div key={s.status} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-xs text-text-secondary">{s.count}</span>
                  <div className={`w-full rounded-t ${cfg?.color.split(' ')[0] || 'bg-brand-blue'}`} style={{ height: `${h}%` }} title={cfg?.label} />
                  <span className="text-xs text-text-muted leading-tight text-center" style={{ fontSize: '9px' }}>{cfg?.label}</span>
                </div>
              );
            })}
          </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="p-8 text-center text-text-secondary">No leads found</td></tr>
            ) : leads.map((l) => {
              const cfg = STATUS_LABELS[l.drip_status] || STATUS_LABELS.registered;
              return (
                <tr key={l.id} className="border-b border-dark-700 hover:bg-dark-700/50 transition-colors">
                  <td className="p-4">
                  ...
                  <td className="p-4 text-text-secondary hidden sm:table-cell">{l.phone || '—'}</td>
                  <td className="p-4 text-text-secondary hidden md:table-cell">{l.country || '—'}</td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${cfg.color}`}>
                      {cfg.label}
                    </span>
                  </td>
                  <td className="p-4 text-text-secondary text-xs hidden lg:table-cell">
                    ...
                    <div className="flex items-center gap-1.5 justify-end">
                      {l.drip_status !== 'converted' && (
                        <button onClick={() => handleConvert(l.id)}
                          ...
                          className="p-1.5 rounded hover:bg-dark-600 text-green-400 hover:text-green-300">
                          <CheckCircle className="w-3.5 h-3.5" />
                        </button>
                      )}
                      <button onClick={() => handleDelete(l.id)}
                        className="p-1.5 rounded hover:bg-dark-600 text-red-400 hover:text-red-300">
