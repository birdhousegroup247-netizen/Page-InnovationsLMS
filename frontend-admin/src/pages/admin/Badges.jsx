    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Award className="w-6 h-6 text-brand-blue" /> Badges
        </h1>
        <div className="flex gap-2">
          <button onClick={fetchAll} className="flex items-center gap-2 px-3 py-2 bg-dark-700 hover:bg-dark-600 text-text-secondary rounded-lg text-sm transition-colors">
            <RefreshCw className="w-4 h-4" />
          </button>
          <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 bg-brand-blue hover:bg-brand-blue-light text-white rounded-lg text-sm font-medium transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-white">{editingId ? 'Edit Badge' : 'New Badge'}</h2>
              <button onClick={() => setShowForm(false)} className="text-text-secondary hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex gap-3">
                <div className="flex-shrink-0">
                  <label className="text-xs text-text-secondary block mb-1">Icon</label>
                  <input
                    type="text"
                    value={form.icon}
                    onChange={(e) => setForm({ ...form, icon: e.target.value })}
                    className="w-14 px-2 py-2 bg-dark-700 border border-dark-600 rounded-lg text-white text-center text-lg focus:outline-none focus:border-brand-blue"
                    maxLength={2}
                  />
                </div>
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-dark-700 border border-dark-600 rounded-lg text-white placeholder-text-secondary text-sm focus:outline-none focus:border-brand-blue"
                  />
                </div>
              </div>
              <div>
                <label className="text-xs text-text-secondary block mb-1">Slug * (unique, no spaces)</label>
                <input
                  type="text"
                  value={form.slug}
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-xs text-text-secondary block mb-1">Trigger</label>
                  <select
                    value={form.condition_type}
                    onChange={(e) => setForm({ ...form, condition_type: e.target.value })}
                    className="w-full px-3 py-2 bg-dark-700 border border-dark-600 rounded-lg text-white text-sm focus:outline-none focus:border-brand-blue"
                  >
                ...
                <div className="w-24">
                  <label className="text-xs text-text-secondary block mb-1">Threshold</label>
                  <input
                    type="number"
                    min={1}
            ...
            <div className="flex gap-3 mt-5">
              <button
                ...
                className="flex-1 px-4 py-2 bg-dark-700 hover:bg-dark-600 text-text-secondary rounded-lg text-sm transition-colors"
              >
                Cancel
              </button>
              <button
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-dark-800 border border-dark-700 rounded-xl">
          <Award className="w-12 h-12 mx-auto text-text-secondary mb-3" />
          <p className="text-text-secondary mb-4">No badges yet. Create your first one.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {badges.map((badge) => (
            <div key={badge.id} className="bg-dark-800 border border-dark-700 rounded-xl p-4 flex items-start gap-3">
              <span className="text-3xl flex-shrink-0">{badge.icon}</span>
              <div className="flex-1 min-w-0">
                <p className="text-white font-medium">{badge.name}</p>
                {badge.description && (
                  <p className="text-text-secondary text-xs mt-0.5 line-clamp-2">{badge.description}</p>
                )}
