...
import {
  MessageSquare, Users, ToggleLeft, ToggleRight,
...
function RoomRow({ room, onToggle, onView }) {
  ...
  return (
    <tr className="border-b border-gray-100 dark:border-dark-700 hover:bg-gray-50 dark:hover:bg-dark-700 transition-colors">
      <td className="px-4 py-3">
        <div>
          <p className="font-medium text-sm text-gray-900 dark:text-white">{room.course?.title}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Instructor: {room.course?.instructor?.full_name || '—'}</p>
        </div>
      </td>
      ...
      <td className="px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400">{formatDate(room.created_at)}</td>
      <td className="px-4 py-3 text-center">
        <span className={cn(
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex items-center justify-end gap-2">
          ...
          >
            {toggling ? '…' : room.is_active ? 'Disable' : 'Enable'}
          </button>
        </div>
      </td>
    </tr>
...
function MessageRow({ msg, onDelete }) {
  ...
  return (
    <div className={cn('flex items-start gap-3 px-4 py-3 border-b border-gray-100 dark:border-dark-700', isDeleted && 'opacity-50')}>
  ...
  if (viewRoom) {
    return (
      <div className="p-6">
        ...
        <div className="mb-6">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">{viewRoom.course?.title}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            All messages · Admin view · {messages.length} total
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 overflow-hidden">
          ...
          {!msgLoading && messages.length === 0 && (
            <p className="text-center text-gray-400 py-10 text-sm">No messages in this room</p>
          )}
          {messages.map((msg) => (
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {total} course chat room{total !== 1 ? 's' : ''} total
          </p>
        </div>
        <button
          onClick={() => loadRooms(page)}
          className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-100 dark:bg-dark-700 text-gray-600 dark:text-gray-300 text-sm hover:bg-gray-200 dark:hover:bg-dark-600 transition-colors"
        >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-4">
          <button
            ...
            className="px-3 py-1.5 rounded-lg text-sm bg-gray-100 dark:bg-dark-700 text-gray-600 dark:text-gray-300 disabled:opacity-40 hover:bg-gray-200 dark:hover:bg-dark-600 transition-colors"
          >
            Prev
          </button>
