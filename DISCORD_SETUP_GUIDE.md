...
- [ ] A Discord server (we call it a "guild") that the LMS will use. If not, create one in Discord: top-left **`+`** → **Create My Own** → name it `TekyPro Community` (or whatever the brand is)
...
3. Name it `TekyPro LMS`. Accept the terms. Click **Create**.
...
3. Paste: `https://<our-railway-url>/api/discord/callback`
   ...
   - For production (later): also add `https://api.tekypro.com/api/discord/callback`
...
5. Open that URL in a new tab. Discord asks which server to add the bot to. Pick the `TekyPro Community` server. Click **Authorise**, complete the captcha.
...
2. Close settings. Right-click the `TekyPro Community` server icon in the left sidebar.
...
> Tip: the bot will need to be placed **above** the Interview Prep role in the Roles list, or it can't assign it. Drag the `TekyPro LMS` bot role above `Interview Prep`. Then drag both above `@everyone`. Save.
...
1. Have a test student account: profile → **Connect Discord** → OAuth flow opens → approve → return to TekyPro
2. The user gets auto-added to the `TekyPro Community` server (you'll see them in the member list)
