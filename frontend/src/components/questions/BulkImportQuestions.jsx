...
/**
 * Instructor bulk CSV import for the question bank.
 * Mirrors the admin BulkImport but posts through questionsAPI.bulkImport
 * — so submissions land as PENDING and wait for admin approval.
 */
export default function BulkImportQuestions({ isOpen, onClose, onSuccess }) {
  ...
  const handleImport = async () => {
    ...
    setLoading(true);
    try {
      const response = await questionsAPI.bulkImport({ questions: preview });
      const created = response.data?.data?.created ?? preview.length;
      showToast(`Submitted ${created} questions for approval`, 'success');
      setStep(3);
      setTimeout(() => {
        onSuccess?.();
        handleClose();
...
"The capital of France is ___",fill_blank,"","Paris","Paris is the capital and largest city of France","2","Geography",easy,1,45,"geography|europe"`;
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Import Questions from CSV" size="xl">
      <div className="p-6">
        {/* Step 1: Upload */}
        {step === 1 && (
          <div>
            <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              ...
              <ul className="text-sm text-blue-800 dark:text-blue-400 space-y-1 list-disc list-inside">
                ...
                <li className="font-medium">All imported questions go to the admin for approval before use.</li>
              </ul>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center hover:border-brand-blue transition-colors">
                <input type="file" accept=".csv" onChange={handleFileSelect} className="hidden" id="csv-upload-instructor" />
                <label htmlFor="csv-upload-instructor" className="cursor-pointer flex flex-col items-center">
            ...
            <div className="mb-6 grid grid-cols-3 gap-4">
              <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium text-green-900 dark:text-green-300">Valid Questions</span>
                </div>
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium text-blue-900 dark:text-blue-300">Total Rows</span>
                </div>
                  ...
                  {preview.map((question, index) => (
                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-dark-700">
                      ...
                      <td className="px-3 py-2">
                        <p className="line-clamp-2 text-gray-900 dark:text-white">{question.question_text}</p>
                      </td>
                      <td className="px-3 py-2">
                        <Badge color="blue">{question.question_type.replace('_', ' ')}</Badge>
                      </td>
                      <td className="px-3 py-2">
                        <Badge color={question.difficulty === 'easy' ? 'green' : question.difficulty === 'medium' ? 'yellow' : 'red'}>
                          {question.difficulty}
                        </Badge>
                      </td>
                      <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{question.marks}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex justify-between gap-3">
              ...
              <div className="flex gap-3">
                ...
                <Button type="button" onClick={handleImport} disabled={loading || preview.length === 0}>
                  {loading ? 'Submitting...' : `Submit ${preview.length} for Approval`}
                </Button>
              </div>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Submitted for Approval</h3>
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              {preview.length} questions were submitted and are now pending admin approval.
            </p>
