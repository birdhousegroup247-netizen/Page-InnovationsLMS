...
class EmailService {
  constructor() {
    ...
    this.from = process.env.EMAIL_FROM || `Page Innovations LMS <${process.env.EMAIL_USER || 'noreply@pageinnovation.com'}>`;
  ...
  async sendWelcomeEmail(email, name) {
    ...
    const html = this._baseTemplate({
      title: 'Welcome to Page Innovations!',
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Welcome to Page Innovations Learning Management System! We're excited to have you join our community of learners.</p>
...
<p>Ready to start learning? Explore our full course catalogue below.</p>`,
    ...
    return this.sendEmail({
      ...
      to: email,
      subject: 'Welcome to Page Innovations LMS!',
      html,
      text: `Welcome to Page Innovations LMS, ${name}! Start your learning journey today.`,
      bypassOptOut: true,
    });
  }
  ...
  async sendVerificationEmail(email, name, token, code) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Welcome to Page Innovations! To finish creating your account, please verify your email address.</p>
...
<p style="font-size:13px;color:#888">If you did not create a Page Innovations account, you can safely ignore this email.</p>`,
    ...
    return this.sendEmail({
      fromKind: 'registration',
      to: email,
      subject: 'Verify your Page Innovations email address',
      html,
      text: `Hi ${name}, verify your Page Innovations email by visiting ${verifyUrl} or by entering this code on the verification page: ${code}`,
      bypassOptOut: true,
    });
  }
  ...
  async sendPasswordResetEmail(email, name, resetToken) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>We received a request to reset the password for your Page Innovations account. Click the button below to set a new password.</p>
...
<p style="font-size:13px;color:#888">Or copy and paste this link into your browser:<br><span style="word-break:break-all;color:#0e2b5c">${resetUrl}</span></p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: 'Password Reset Request - Page Innovations LMS',
      html,
...
<p>Share your achievement on LinkedIn and inspire someone else to start. And keep the momentum going — the next course is only a click away.</p>`,
  ...
  async sendTestAssignmentEmail(email, name, test) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>You have a new test on Page Innovations:</p>
<div class="hi">
...
<p>Good luck!</p>`,
  ...
  async sendInstructorApplicationReceived(email, name) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Thanks for applying to teach on Page Innovations. We've received your application and our admin team is reviewing it.</p>
...
<li>In the meantime, you can use Page Innovations as a student — browse courses, watch previews, and join the community</li>
</ul></div>
<p>If you have any questions while you wait, just reply to this email.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: 'Your Page Innovations instructor application is being reviewed',
      html,
...
<p>A new instructor application has been submitted on Page Innovations and is awaiting review.</p>
<div class="hi">
...
<p>Open the admin portal to review the bio, qualifications, teaching experience, and supporting documents (CV + credentials).</p>`,
  ...
  async sendInstructorApprovalEmail(email, name) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Great news — your instructor application has been <strong>approved</strong>. Welcome to the Page Innovations instructor community!</p>
...
<p>Ready to create your first course?</p>`,
  ...
  async sendInstructorRejectionEmail(email, name, reason) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Thank you for your interest in becoming a Page Innovations instructor.</p>
...
<p>You can still enjoy all student features and continue learning on our platform.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: 'Instructor Application Update - Page Innovations LMS',
      html,
...
<p>Your instructor privileges have been removed from your Page Innovations account.</p>
...
<p>If you believe this action was taken in error, please contact our support team.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: 'Instructor Status Update - Page Innovations LMS',
      html,
...
${announcement.instructor_name ? `<p style="margin:8px 0 0;color:#555"><em>— ${announcement.instructor_name}</em></p>` : ''}
  ...
  _baseTemplate({ headerColor = 'linear-gradient(135deg,#0e2b5c,#2e3192)', title, body, ctaText, ctaUrl, unsubUrl, transactional = false }) {
    ...
    const unsubLine = transactional
      ? ''
      : `<p style="margin-top:10px;font-size:11px;color:#bbb">You received this because you registered on Page Innovations. <a href="${unsubUrl || `${FE}/unsubscribe`}">Unsubscribe</a></p>`;
...
<div class="hdr"><p class="logo">Page Innovations</p><h1>${title}</h1></div>
<div class="bd">${body}${cta}</div>
<div class="ft"><p>Page Innovations — Professional Database Training</p><p><a href="https://www.pageinnovation.com">www.pageinnovation.com</a> · <a href="mailto:support@pageinnovation.com">support@pageinnovation.com</a></p>${unsubLine}</div>
</div></div></body></html>`;
  }
  ...
  async sendLeadWelcome(email, name, courseTitle = 'our courses') {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Welcome to Page Innovations! Your account is all set — you now have <strong>free preview access</strong> to all our courses.</p>
...
<p>Start exploring now 👇</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Welcome to Page Innovations, ${name} — your free preview is ready`, html, text: `Hi ${name}, your Page Innovations free preview is ready. Browse courses at ${FE}/courses` });
  }
  ...
  async sendLeadFollowupD1(email, name, courseTitle = 'your course') {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Just checking in — <strong>${courseTitle}</strong> is ready and waiting for you on Page Innovations.</p>
<p>Thousands of database professionals have used Page Innovations to advance their careers, pass certification exams, and land better-paying roles.</p>
...
<p>Ready to take the next step?</p>`,
    ...
    return this.sendEmail({ to: email, subject: `${name}, your Page Innovations course is waiting for you`, html, text: `Hi ${name}, your course on Page Innovations is ready. Continue at ${FE}/courses` });
  }

  async sendLeadFollowupD3(email, name) {
    const FE = process.env.FRONTEND_URL || 'http://localhost:5173';
    const html = this._baseTemplate({
      title: 'What Page Innovations students are saying',
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>We thought you'd like to hear from some of our graduates:</p>
<div class="hi"><p><em>"I passed my Oracle DBA certification after completing the Page Innovations course. The hands-on practice labs made all the difference."</em><br>— <strong>Adesola K., Lagos</strong></p></div>
<div class="hi"><p><em>"I was able to negotiate a 40% salary increase after completing my PostgreSQL certification through Page Innovations. Worth every penny."</em><br>— <strong>James M., Nairobi</strong></p></div>
...
<p>Your free preview is still active. Come see what the full course looks like.</p>`,
    ...
    return this.sendEmail({ to: email, subject: `What Page Innovations students say (real results)`, html, text: `Hi ${name}, see what Page Innovations students are saying at ${FE}/courses` });
  }
  ...
  async sendLeadFollowupD7(email, name) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Your Page Innovations free preview has been active for a week now. We want to make sure you're making the most of it before time runs out.</p>
...
<p>Remember — you can start with just <strong>60% upfront</strong> and pay the rest in 21 days. No hidden fees, no surprises.</p>`,
  ...
  async sendLeadFollowupD14(email, name, couponCode = null) {
    ...
    const discountBlock = couponCode
      ...
      : '';
    const html = this._baseTemplate({
      title: 'A personal note from Page Innovations',
      body: `<p>Hi <strong>${name}</strong>,</p>
...
<p>You now have full access to your course. Start learning right now!</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: `Page Innovations Payment Receipt — ${courseTitle}`,
      html,
  ...
  async sendPaymentCongrats(email, name, { courseTitle, courseId }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Congratulations! You are now enrolled in <strong>${courseTitle}</strong> on Page Innovations.</p>
<p>Here's how to get started:</p>
<div class="hi"><ol style="padding-left:20px;line-height:2">
<li><strong>Log in</strong> to your Page Innovations account</li>
...
<p>We're rooting for you. Let's go!</p>`,
  ...
  async sendOnboardingD1(email, name, { courseTitle, courseId }) {
    const FE = process.env.FRONTEND_URL || 'http://localhost:5173';
    const html = this._baseTemplate({
      title: 'Your first 3 steps on Page Innovations',
      body: `<p>Hi <strong>${name}</strong>,</p>
...
<p>You've got this. We'll check in with you in a couple of days.</p>`,
  ...
  async sendOnboardingD3(email, name, { courseTitle }) {
    const FE = process.env.FRONTEND_URL || 'http://localhost:5173';
    const html = this._baseTemplate({
      title: 'Did you know Page Innovations has these features?',
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>You've been with us for 3 days. How are things going in <strong>${courseTitle}</strong>?</p>
<p>Here are some Page Innovations features many students discover <em>too late</em> — don't let that be you:</p>
...
<p>Keep going — you're doing great!</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Did you know Page Innovations has these features? (${courseTitle})`, html, text: `Hi ${name}, discover Page Innovations's hidden features. Continue at ${FE}/my-courses` });
  }
  ...
  async sendOnboardingD7(email, name, { courseTitle }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>You've been on Page Innovations for a week. We just wanted to check in on your progress in <strong>${courseTitle}</strong>.</p>
...
<p>— The Page Innovations Team</p>`,
  ...
  async sendInstallmentReminderD21(email, name, { remainingAmount, dueDate, payUrl, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Just a friendly heads-up — your Page Innovations installment balance of <strong>${amt}</strong> is now due.</p>
...
<p>No rush — but sooner is better! 😊</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Friendly reminder: your Page Innovations balance of ${amt} is due`, html, text: `Hi ${name}, your Page Innovations balance of ${amt} is due. Pay at ${payUrl}`, bypassOptOut: true });
  }
  ...
  async sendInstallmentReminderD24(email, name, { remainingAmount, dueDate, payUrl, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Your Page Innovations installment balance of <strong>${amt}</strong> was due on ${new Date(dueDate).toLocaleDateString('en-US', { dateStyle: 'long' })} and is now 3 days overdue.</p>
...
<p>It only takes a moment to sort this out:</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Your Page Innovations balance is 3 days overdue — still time to sort it out`, html, text: `Hi ${name}, your Page Innovations balance of ${amt} is 3 days overdue. Pay at ${payUrl}`, bypassOptOut: true });
  }
  ...
  async sendInstallmentReminderD28(email, name, { remainingAmount, payUrl, lockDate, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>We need to let you know that your Page Innovations account will be <strong>partially restricted on ${new Date(lockDate).toLocaleDateString('en-US', { dateStyle: 'long' })}</strong> if your balance remains unpaid.</p>
...
<p>Your progress is always saved — one payment click restores everything instantly.</p>`,
    ...
    return this.sendEmail({ to: email, subject: `URGENT: Your Page Innovations account will be restricted in 4 days`, html, text: `URGENT: Your Page Innovations balance of ${amt} is overdue. Account restricted on ${lockDate}. Pay at ${payUrl}`, bypassOptOut: true });
  }
  ...
  async sendInstallmentReminderD32(email, name, { remainingAmount, payUrl, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Because your installment balance of <strong>${amt}</strong> remains unpaid, your Page Innovations account has been <strong>partially restricted</strong>.</p>
...
<p>Restore full access instantly with one payment:</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Your Page Innovations account has been partially restricted`, html, text: `Hi ${name}, your Page Innovations account is partially restricted. Pay ${amt} to restore full access at ${payUrl}`, bypassOptOut: true });
  }
  ...
  async sendInstallmentReminderD35(email, name, { remainingAmount, payUrl, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Your Page Innovations account is now on hold due to an outstanding balance of <strong>${amt}</strong>.</p>
...
<p>One click is all it takes to restore everything instantly:</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Your Page Innovations account is on hold — here's how to restore it`, html, text: `Hi ${name}, your Page Innovations account is on hold. Pay ${amt} to restore at ${payUrl}`, bypassOptOut: true });
  }
  ...
  async sendInstallmentSuspendedD42(email, name, { remainingAmount, payUrl, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      transactional: true,
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Your Page Innovations account has been suspended due to an unpaid balance of <strong>${amt}</strong>.</p>
...
<p>To reactivate your account and restore full access <em>immediately</em>, complete your payment below:</p>`,
    ...
    return this.sendEmail({ to: email, subject: `Your Page Innovations account has been suspended`, html, text: `Hi ${name}, your Page Innovations account has been suspended. Pay ${amt} to reactivate at ${payUrl}`, bypassOptOut: true });
  }
  ...
  async sendChatNotificationEmail(email, name, type, senderName, preview) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>${type === 'mention'
        ? `<strong>${senderName}</strong> mentioned you in a chat on Page Innovations.`
...
<p style="font-size:12px;color:#888">You received this because you were not online at the time.</p>`,
  ...
  async sendDiscordInviteEmail(email, name, { courseTitle, inviteLink }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Welcome to the Page Innovations community on Discord! Your private channel for <strong>${courseTitle}</strong> is now ready.</p>
<div class="hi">
...
<p>You can also access this link anytime from your course page in the Page Innovations app.</p>`,
      ctaText: 'Join Discord Channel',
      ctaUrl: inviteLink,
    });
    return this.sendEmail({
      to: email,
      subject: `Your Discord channel for ${courseTitle} is ready — Page Innovations`,
      html,
...
<p>If you have any questions, reply to this email or contact our support team.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: `Your Page Innovations refund for ${courseTitle} has been processed`,
      html,
...
<p>Everyone at Page Innovations is rooting for you today. 🎂</p>
...
<p style="font-size:18px">💙 — The Page Innovations Team</p>`,
      ctaText: 'Open Page Innovations',
      ctaUrl: `${FE}/dashboard`,
    });
    return this.sendEmail({
      to: email,
      ...
      html,
      text: `Happy Birthday, ${firstName}! Everyone at Page Innovations is wishing you a great one. — The Page Innovations Team`,
      recipientKind: 'user',
    });
  }
...
<div class="hi"><p>Grab a drink, close a tab or two, and get comfortable. We'll see you there!</p></div>`,
      ctaText: joinUrl ? 'Join Live Session' : 'Open Page Innovations',
...
<div class="wa"><p>If you've already submitted, thanks! You can ignore this. Otherwise, hop in and get it done.</p></div>`,
  ...
  async sendReEngagementEmail(email, name, { courseTitle, courseId, daysInactive }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${firstName}</strong>,</p>
<p>It's been about ${daysInactive} days since we last saw you on Page Innovations${courseTitle ? `, and <strong>${courseTitle}</strong> is right where you left it` : ''}.</p>
<p>We know life gets busy. Here's a small nudge:</p>
<div class="hi"><p><strong>Just 20 minutes today</strong> — one lesson, one practice question. That's how every certificate on Page Innovations gets earned.</p></div>
...
<p>We're rooting for you.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: `${firstName}, your Page Innovations course is still waiting`,
      html,
  ...
  async sendInstructorFirstCoursePublishedEmail(email, name, { courseTitle, courseId }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Big moment — <strong>${courseTitle}</strong> is now published and visible to every student on Page Innovations. Welcome to the instructor roster!</p>
...
<p>We'll be watching your dashboard and cheering when the first enrollment lands.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: `Your course "${courseTitle}" is live on Page Innovations!`,
      html,
  ...
  async sendInstructorMonthlyEarnings(email, name, { monthLabel, grossRevenue, enrollments, refunds, netRevenue, topCourse, currency = 'USD' }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Here's your monthly Page Innovations summary for <strong>${monthLabel}</strong>:</p>
<div class="hi">
...
<p>Full breakdown and per-course numbers are in the instructor dashboard.</p>`,
    ...
    return this.sendEmail({
      to: email,
      subject: `Your ${monthLabel} Page Innovations earnings — ${fmtMoney(netRevenue, currency)}`,
      html,
      text: `Hi ${name}, ${monthLabel} on Page Innovations: ${fmtMoney(grossRevenue, currency)} gross, ${enrollments} new enrollments, ${fmtMoney(netRevenue, currency)} net. Details at ${FE}/instructor/dashboard`,
      recipientKind: 'user',
    });
  }
  ...
  async sendInstructorReviewMilestone(email, name, { milestone, averageRating }) {
    ...
    const html = this._baseTemplate({
      ...
      body: `<p>Hi <strong>${name}</strong>,</p>
<p>Big milestone: you just crossed <strong>${milestone} student reviews</strong> on Page Innovations${averageRating ? ` at an average rating of <strong>${Number(averageRating).toFixed(2)} / 5.0</strong>` : ''}.</p>
...
<p>Keep going. Every great instructor was once a first-course instructor.</p>`,
    ...
    return this.sendEmail({
      ...
      html,
      text: `Hi ${name}, congrats — you just passed ${milestone} student reviews on Page Innovations. See them: ${FE}/instructor/dashboard`,
      recipientKind: 'user',
    });
  }
  ...
  async sendCertificateShareNudge(email, name, { courseTitle, courseId, certificateUrl, suggestedCourseTitle, suggestedCourseId }) {
    const FE = process.env.FRONTEND_URL || 'http://localhost:5173';
    const linkedInText = encodeURIComponent(`Just completed ${courseTitle} on Page Innovations! 🎓`);
...
${suggestedCourseTitle ? `<p>And when you're ready for what's next: <strong>${suggestedCourseTitle}</strong> is a natural follow-up to what you just finished. Take a look when you have a moment.</p>` : '<p>Ready for what\'s next? Browse the catalog for your next challenge.</p>'}`,
  ...
  async sendPromotionalEmail(email, name, { subject, title, bodyHtml, ctaText, ctaUrl, recipientKind = 'user', recipientId }) {
    const html = this._baseTemplate({
      ...
      title: title || 'A note from Page Innovations',
      body: bodyHtml,
      ctaText,
      ctaUrl,
