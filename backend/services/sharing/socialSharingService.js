...
class SocialSharingService {
   ...
   */
  static generateCourseShareLinks(course, baseUrl = 'https://www.tekypro.com') {
    ...
    return {
      ...
      twitter: `https://twitter.com/intent/tweet?text=${title}&url=${encodeURIComponent(courseUrl)}&via=TekyPro`,
   ...
   */
  static generateCertificateShareLinks(certificate, student, course, baseUrl = 'https://www.tekypro.com') {
    const certificateUrl = `${baseUrl}/certificates/verify/${certificate.certificate_unique_id}`;
    const title = encodeURIComponent(`I completed ${course.title} on TekyPro!`);
    const message = encodeURIComponent(
      `I'm proud to share that I've completed ${course.title} on TekyPro LMS. Check out my certificate!`
    );

    return {
      facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(certificateUrl)}`,
      twitter: `https://twitter.com/intent/tweet?text=${message}&url=${encodeURIComponent(certificateUrl)}&hashtags=Learning,Certificate,TekyPro`,
   ...
   */
  static generateLinkedInCertificationShare(certificate, student, course, baseUrl = 'https://www.tekypro.com') {
    const params = new URLSearchParams({
      name: course.title,
      organizationId: '0', // TekyPro organization ID (you'll need to register)
   ...
   */
  static generateCourseOGMetadata(course, baseUrl = 'https://www.tekypro.com') {
    return {
   ...
   */
  static generateCertificateOGMetadata(certificate, student, course, baseUrl = 'https://www.tekypro.com') {
    return {
      'og:title': `${student.full_name} - ${course.title} Certificate`,
      'og:description': `${student.full_name} has successfully completed ${course.title} on TekyPro LMS`,
      ...
      'twitter:description': `${student.full_name} completed this course on TekyPro`,
    };
  }
  ...
  static generateShareText(type, data) {
    const templates = {
      course_completed: `I just completed "${data.courseTitle}" on @TekyPro! 🎉 Check it out: ${data.url}`,
      course_enrolled: `Just enrolled in "${data.courseTitle}" on @TekyPro! Excited to learn! 📚 ${data.url}`,
      certificate_earned: `Proud to share that I've earned my certificate for "${data.courseTitle}" from @TekyPro! 🏆 ${data.url}`,
      achievement: `New achievement unlocked on @TekyPro! ${data.achievementName} 🎯 ${data.url}`,
    };
