...
class AssignmentsController {
  ...
  static async createAssignment(req, res, next) {
    try {
      const { courseId } = req.params;
      const { title, description, due_date, max_score, allow_file_upload, allow_text_submission, content_id } = req.body;
  ...
  static async updateAssignment(req, res, next) {
    try {
      ...
      const { title, description, due_date, max_score, allow_file_upload, allow_text_submission } = req.body;
      await assignment.update({ title, description, due_date, max_score, allow_file_upload, allow_text_submission });
  ...
  static async submitAssignment(req, res, next) {
    try {
      const { id } = req.params;
      const { text_content, file_url, file_name } = req.body;
      ...
      if (!text_content?.trim() && !file_url) return ApiResponse.error(res, 'Provide text or file', 400);
  ...
  static async updateSubmission(req, res, next) {
    try {
      ...
      const { text_content, file_url, file_name } = req.body;
      await submission.update({ text_content, file_url, file_name });
